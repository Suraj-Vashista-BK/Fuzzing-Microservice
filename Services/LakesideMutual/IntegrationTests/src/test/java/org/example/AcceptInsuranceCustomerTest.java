...
public class AcceptInsuranceCustomerTest {

    HttpResponse patchHttpHelper(JSONObject data, String url, String token) throws Exception{
    ...
    @Test
    public void acceptInsuranceTest() throws Exception{
        String email = "admin@example.com";
        String password = "1password";
        String insuranceType = "Life Insurance";
        String status = "REQUEST_SUBMITTED";
        String status2 = "QUOTE_RECEIVED";
        String status3 = "QUOTE_ACCEPTED";
        ...
        String custQuoteUrl = "http://localhost:8080/customers/";
...
//      Request policy quote from customer
        ...
        insuranceInfo.put("insuranceType", "Life Insurance");
...
//        Verify status of request at admin end
        respJson = parseResponse(response);
...
//      get status of request from customer end
        custQuoteUrl = custQuoteUrl + customerId + "/insurance-quote-requests";
        response = getHttpHelper(custQuoteUrl, responseToken);
        responseCode = Integer.valueOf(String.valueOf(response.statusCode()));
        JSONArray respArray = new JSONArray(response.body().toString());

        assertTrue(respArray.length()>0);
        assertEquals(Integer.valueOf(200), responseCode);
...
//            Customer accepts insurance
            ...
            acceptData.put("status", "QUOTE_ACCEPTED");
            String patchInsUrl = insUrl + "/" + requestId;
            response = patchHttpHelper(acceptData, patchInsUrl, responseToken);
            responseCode = Integer.valueOf(String.valueOf(response.statusCode()));
            assertEquals(Integer.valueOf(200), responseCode);
...
//            Verify status
            ...
            jsonarray = (JSONArray) respJson.get("statusHistory");
            statusObj = new JSONObject();
            statusObj = jsonarray.getJSONObject(2);
            insurance = new JSONObject();
            insurance = respJson.getJSONObject("insuranceOptions");

            assertEquals(Integer.valueOf(200), responseCode);
            assertEquals(status3, statusObj.get("status"));
            assertEquals(insuranceType, insurance.get("insuranceType"));
